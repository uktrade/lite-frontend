...
function setSelectButtonsState() {
	$( 'table' ).each(function(i, obj) {
		...
		if ($(obj).find('input:checkbox:checked').length == $(obj).find('input:checkbox').length) {
			$(obj).find('.lite-button-checkbox').addClass( 'lite-button-checkbox--checked' );
		} else if ($(obj).find('input:checkbox:checked').length != 0) {
			$(obj).find('.lite-button-checkbox').addClass( 'lite-button-checkbox--indeterminate' );
		}
