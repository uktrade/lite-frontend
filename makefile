...
caseworker-e2e-selenium-test:
	...
	$(docker-e2e-caseworker) exec caseworker bash -c '$(wait-for-caseworker) && pipenv run pytest --circleci-parallelize --headless --chrome-binary-location=/usr/bin/chromium -vv --gherkin-terminal-reporter --junitxml=test_results/output.xml ./ui_tests/caseworker'

exporter-e2e-selenium-test:
	@echo "*** Requires starting the exporter stack, which can be started running: 'make start-exporter' ***"
	$(docker-e2e-exporter) exec exporter bash -c '$(wait-for-exporter) && pipenv run pytest --circleci-parallelize --headless --chrome-binary-location=/usr/bin/chromium -vv --gherkin-terminal-reporter --junitxml=test_results/output.xml ./ui_tests/exporter'
