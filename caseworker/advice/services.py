...
def remove_lu_countersign_flags(request, case, queue_id, caseworker, order, outcome_accepted):
    """
    First countersign
        Case Type 1.1: `outcome_accepted` is `Yes` and `LU_SR_MGR_CHECK_REQUIRED` flag is set
        - try remove `LU_COUNTERSIGN_REQUIRED` flag on destination
        - try remove `AP_LANDMINE` flag on good

        Case Type 1.2: `outcome_accepted` is `Yes` and `LU_SR_MGR_CHECK_REQUIRED` flag is NOT set
        - try remove `LU_COUNTERSIGN_REQUIRED` flag on destination
        - try remove `AP_LANDMINE` flag on good
        - try remove `MANPADS` flag on good

        Case Type 1.3: `outcome_accepted` is `No`
        - try remove `LU_COUNTERSIGN_REQUIRED` flag on destination
        - try remove `LU_SR_MGR_CHECK_REQUIRED` flag on destination
        - try remove `AP_LANDMINE` flag on good
        - try remove `MANPADS` flag on good
