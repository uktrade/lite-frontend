...
def new_queue_form(request):
    return Form(
        ...
        back_link=BackLink(AddQueueForm.BACK, reverse("queues:manage")),
    )
        ...
        questions=[
            ...
            Select(
                ...
                options=remove_current_queue_id(
                    get_queues(request=request, disable_pagination=True, convert_to_options=True, users_team_first=True),
                    queue_id,
                ),
                name="countersigning_queue",
            ),
        ],
        back_link=BackLink(EditQueueForm.BACK, reverse("queues:manage")),
    )
