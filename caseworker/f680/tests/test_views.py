...
class TestECJUQueryView:
    ...
    def test_GET_new_success(self, authorized_client, mock_f680_case):
        response = authorized_client.get(self.new_url)
        assert response.status_code == HTTPStatus.OK

    def test_GET_close_success(self, authorized_client, mock_f680_case):
        response = authorized_client.get(self.close_url)
        assert response.status_code == HTTPStatus.OK

    def test_GET_close_error_query_not_found(self, authorized_client, mock_f680_case):
        close_url = reverse(
            "cases:f680:close_ecju_query",
            kwargs={
    ...
    def test_POST_new_query_form_error(self, authorized_client, mock_f680_case, mock_post_new_ecju_query):
        response = authorized_client.post(self.new_url, data={})
        assert response.status_code == HTTPStatus.OK
        assert response.context["form"].errors == {"question": ["This field is required."]}

    def test_POST_close_query_form_error(self, authorized_client, mock_f680_case, mock_put_close_ecju_query):
    ...
    def test_POST_close_query_success(
        self, authorized_client, mock_f680_case, mock_put_close_ecju_query, gov_uk_user_id
    ):
        response = authorized_client.get(self.close_url)
        assert response.status_code == HTTPStatus.OK
