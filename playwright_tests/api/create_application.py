...
def create_application():
    ...
    create_goods_document(good_id, [document('goods document', 'some goods document')], exporter(export_user_token, organisation_id))

    # Add goods to application
    add_good_to_application(application_id, {**goods(), 'good_id': good_id}, exporter(export_user_token, organisation_id))
    ...
    add_end_user_to_draft_response = add_end_user_to_application(application_id, end_user, exporter(export_user_token, organisation_id))
    end_user_id = add_end_user_to_draft_response['end_user']['id']
    
    # Add document to end user
    add_document_to_application_end_user(application_id, end_user_id, document('party document', 'some party document'), exporter(export_user_token, organisation_id))

    # Add ultimate end user to application
    add_ultimate_end_user_to_application(application_id, ultimate_end_user, exporter(export_user_token, organisation_id))
    ...
    submitted_application_response = submit_application(application_id, submit, exporter(export_user_token, organisation_id))
