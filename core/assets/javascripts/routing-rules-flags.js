(function () {
  ...
  function renderTable(conditions) {
    if (
      conditions &&
      conditions.flags_to_include.length === 0 &&
      conditions.flags_to_exclude.length === 0
    ) {
      $("#routing-rules-flags-details").hide();
      return;
    }
    edit_column_header = editing
    ...
    if (conditions.flags_to_include.length) {
      ...
      var edit_column_data = editing
        ? '<td class="govuk-table__cell govuk-table__cell"><a id="edit-inclusive-flags" class="condition-edit" href="#" style="color:#1D70B8">Edit</a></td>'
        : "";

      rows.push(`
                <tr class="govuk-table__row">
                    <td class="govuk-table__cell govuk-table__cell">Applies to cases that contain selected flags</td>
                    <td class="govuk-table__cell govuk-table__cell">${flags.join(
                      ", "
                    )}</td>
                    ${edit_column_data}
      ...
      var edit_column_data = editing
        ? '<td class="govuk-table__cell govuk-table__cell"><a id="edit-inclusive-flags" class="condition-edit" href="#" style="color:#1D70B8">Add</a></td>'
        : "";
      rows.push(`
                <tr class="govuk-table__row">
                    <td class="govuk-table__cell govuk-table__cell">Applies to cases that contain selected flags</td>
    ...
    if (conditions.flags_to_exclude.length) {
      ...
      var edit_column_data = editing
        ? '<td class="govuk-table__cell govuk-table__cell"><a id="edit-exclusive-flags" class="condition-edit" href="#" style="color:#1D70B8">Edit</a></td>'
        : "";
      rows.push(`
                <tr class="govuk-table__row">
      ...
      var edit_column_data = editing
        ? '<td class="govuk-table__cell govuk-table__cell"><a id="edit-exclusive-flags" class="condition-edit" href="#" style="color:#1D70B8">Add</a></td>'
        : "";
      rows.push(`
                <tr class="govuk-table__row">
  ...
  function resetControls(editing) {
    ...
    $("#routing_rules_flags_condition-contain_selected_flags").prop(
      "checked",
      false
    );
    $("#routing_rules_flags_condition-contain_selected_flags").prop(
    ...
    $("#routing_rules_flags_condition-doesnot_contain_selected_flags").prop(
      "checked",
      false
    );
    $("#routing_rules_flags_condition-doesnot_contain_selected_flags").prop(
    ...
    $('a:contains("Add another condition")')
      .show()
  ...
  function updateFlagInputFields(conditions) {
    var includingIds = conditions.flags_to_include.map(function (flag) {
      return flag.id;
    });
    var excludingIds = conditions.flags_to_exclude.map(function (flag) {
      return flag.id;
    });
  ...
  function editFlags(event) {
    ...
    if (event.target.id === "edit-inclusive-flags") {
      $("#condition-label").text(
        "The routing rule will be applied to cases that contain your selected flags"
      );
      $("#routing_rules_flags_condition-contain_selected_flags").prop(
        "checked",
        true
      ...
      $("#condition-label").text(
        "The routing rule will be applied to cases that do not contain your selected flags"
      );
      $("#routing_rules_flags_condition-doesnot_contain_selected_flags").prop(
        "checked",
        true
  ...
  function removeFlags(event) {
    ...
    if (
      flagGroups.flags_to_include.length === 0 &&
      flagGroups.flags_to_exclude.length === 0
    ) {
  ...
  function setFlagsCheckedStatus(flags, status) {
    ...
    var allFlags = $("input[type='checkbox']")
      .map(function () {
        return this;
      })
      .toArray();
    ...
    if (condition) {
      if (condition === "contain_selected_flags") {
        if (append) {
          ...
          var currentFlagIds = flagGroups.flags_to_include.map(
            (flag) => flag.id
          );
      ...
      if (condition === "doesnot_contain_selected_flags") {
        if (append) {
          // add the flag only if it is not included already
          var currentFlagIds = flagGroups.flags_to_exclude.map(
            (flag) => flag.id
          );
          for (var i = 0; i < selectedFlags.length; i++) {
            if (!currentFlagIds.includes(selectedFlags[i].id)) {
  ...
  function getSelectedFlags() {
    var selectedFlags = $("input[type='checkbox']:checked")
      .map(function () {
  ...
  function getSelectedCondition() {
    if (
      $("#routing_rules_flags_condition-contain_selected_flags").is(":checked")
    ) {
      return "contain_selected_flags";
    } else if (
      $("#routing_rules_flags_condition-doesnot_contain_selected_flags").is(
        ":checked"
      )
    ) {
  ...
  $("#routing_rules_flags_condition-contain_selected_flags").change(
    function () {
      setFlagsDisabledStatus(flagGroups.flags_to_exclude, true);
      setFlagsCheckedStatus(flagGroups.flags_to_include, true);
    }
  ...
  $("#routing_rules_flags_condition-doesnot_contain_selected_flags").change(
    function () {
      setFlagsDisabledStatus(flagGroups.flags_to_include, true);
      setFlagsCheckedStatus(flagGroups.flags_to_exclude, true);
    }
  ...
  $('a:contains("Add another condition")').click(function () {
    ...
    if ($(".govuk-error-summary")[0] || $(".govuk-error-message")[0]) {
      ...
      if (
        condition !== "none" &&
        (flagGroups.flags_to_include.length ||
          flagGroups.flags_to_exclude.length)
      ) {
        $(".govuk-error-summary").hide().children().hide();
        $(".govuk-error-message").hide();
        $("#pane_routing_rules_flags_condition").removeClass(
  ...
  function setInitialState(editing) {
    flags_to_include = $('input[name="flags_to_include"]')
      .val()
      .split(",")
      .filter(function (el) {
        return el;
      });
    flags_to_exclude = $('input[name="flags_to_exclude"]')
      .val()
      .split(",")
      .filter(function (el) {
        return el;
