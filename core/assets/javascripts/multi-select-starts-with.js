...
const tokenFieldSetup = (element, items, selected) => {
  var tokenField = new LiteTokenFieldCustomSearch({
    el: element,
    items: items,
    newItems: false,
    addItemOnBlur: true,
  ...
  tokenField.fieldInput = "";
  tokenField.originalItems = items;
  element.remove();
  return tokenField;
};
...
const progressivelyEnhanceMultipleSelectField = (
  ...
  var tokenField = tokenFieldSetup(element, items, selected);
  return tokenField;
};
