...
@pytest.mark.parametrize(
    ("case_type", "order", "outcome_accepted", "flags_before", "flags_after", "queue_details"),
    (
        (
            "Case Type 1.1",
...
def test_remove_lu_countersign_flags(
    ...
    # Set up flags on the case
    flags = []
    if services.LU_COUNTERSIGN_REQUIRED in flags_before:
        flags.append(flag_lu_countersign_required)
    if services.LU_SR_MGR_CHECK_REQUIRED in flags_before:
        flags.append(flag_lu_senior_manager_check_required)
    if services.AP_LANDMINE in flags_before:
        flags.append(flag_ap_landmine)
    if services.MANPADS in flags_before:
        flags.append(flag_manpads)
    data_standard_case["case"]["all_flags"] = flags
    data_standard_case["case"]["flags"] = flags

    # Set up mock API requests
    countersign_advice_url = client._build_absolute_uri(f"/cases/{case_id}/countersign-decision-advice/")
    requests_mock.post(countersign_advice_url, json={})
    case_url = reverse("cases:case", kwargs={"queue_pk": queue_details["id"], "pk": case_id})
    requests_mock.get(client._build_absolute_uri(case_url), json=data_standard_case)
    ...
    # Get grouped user advice for countersigning
    advice_to_countersign = services.get_advice_to_countersign(advice_for_countersign, current_user)

    # Put request to case queues url
    case_queues_url = client._build_absolute_uri(f"/cases/{case_id}/queues/")
    requests_mock.put(case_queues_url, json={})

    # Set up form data part 1
    data = {
        "form-TOTAL_FORMS": [f"{len(advice_to_countersign)}"],
        "form-INITIAL_FORMS": ["0"],
        "form-MIN_NUM_FORMS": [f"{len(advice_to_countersign)}"],
    ...
    # Assert the flags on the case are as expected
    flags_url = reverse(f"cases:assign_flags", kwargs={"queue_pk": queue_id, "pk": case_id}) + f"?case={case_id}"
    response = authorized_client.get(flags_url, data={})
    soup = BeautifulSoup(response.content, "html.parser")

    # TODO: Assert flags are equal to those in `flags_before`

    # From this point the fixture `with_lu_countersigning_enabled` is needed

    # Actually send the countersign data
    countersign_decision_url = reverse(
        f"cases:countersign_decision_review",
        kwargs={"queue_pk": queue_details["id"], "pk": case_id},
    )
    ...
    # TODO: Assert flags are equal to those in `flags_after`
