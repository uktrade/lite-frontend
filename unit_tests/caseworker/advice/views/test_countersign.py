...
@patch("caseworker.advice.views.get_gov_user")
def test_other_recommendations_expanded_lu_team(
    mock_get_gov_user,
    authorized_client,
    data_standard_case,
    final_advice,
    ...
    for d in list_of_details:
        assert d.as_attr("open")


@patch("caseworker.advice.views.get_gov_user")
def test_other_recommendations_not_expanded_not_in_lu_team(
    mock_get_gov_user,
    authorized_client,
    data_standard_case,
    current_user,
    ...
    # Setup mock API requests
    mock_get_gov_user.return_value = (
        {"user": {"team": {"id": current_user["team"]["id"], "alias": current_user["team"]["alias"]}}},
        None,
    )
