...
@pytest.fixture
def case_url(data_standard_case):
...
def test_appeal_view(authorized_client, appeal_url, case_url):
    ...
    assert soup.find("a", {"id": "back-link"})["href"] == case_url
    assert soup.find("input", {"id": "submit-id-submit"})["value"] == "Submit appeal request"
    assert soup.find("a", {"id": "cancel-id-cancel"})["href"] == case_url
