...
def test_add_complete_item_to_application_end_to_end_no_set_quantities_or_values(
    requests_mock,
    expected_good_data,
    mock_good_on_application_post,
    application,
...
def test_add_complete_item_to_application_end_to_end_handles_service_error(
    ...
    assert response.status_code == 200
    assert "Unexpected error adding complete item to application" in str(response.content)
