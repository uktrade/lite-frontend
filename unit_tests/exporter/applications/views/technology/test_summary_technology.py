...
def test_technology_summary_template_used(
    ...
    response = authorized_client.get(technology_product_summary_url)
    assertTemplateUsed(response, "applications/goods/technology/product-summary.html")
...
def test_technology_product_summary_context(
    ...
    def _get_test_url(name):
        if not name:
            return None
        return f'/applications/{data_standard_case["case"]["id"]}/goods/{good_id}/technology/edit/{name}/'
