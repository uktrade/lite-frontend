...
class SeedParty(seed_class.SeedClass):
    def add_document(self, url):
        doc_s3_key = manage_s3_documents.upload_test_document_to_aws(self.base_url)
        data = request_data.create_document('kebab', 'tasty', doc_s3_key)
        make_requests.make_request("POST", base_url=self.base_url, url=url, headers=self.export_headers, body=data)
    ...
    def add_eua_query(self):
        ...
        id = make_requests.make_request("POST", base_url=self.base_url, url='/queries/end-user-advisories/',
                                        headers=self.export_headers, body=data).json()['end_user_advisory']['id']
        self.add_to_context('end_user_advisory_id', str(id))
        case_id = make_requests.make_request("GET", base_url=self.base_url, url='/queries/end-user-advisories/' + str(id) + '/',
                                             headers=self.export_headers).json()['case_id']
    ...
    def add_end_user(self, draft_id, enduser):
        ...
        end_user = make_requests.make_request("POST", base_url=self.base_url, url='/applications/' + draft_id + '/end-user/',
    ...
    def add_ultimate_end_user(self, draft_id, ultimate_end_user):
        ...
        ultimate_end_user_post = make_requests.make_request('POST', base_url=self.base_url,
    ...
    def add_consignee(self, draft_id, consignee):
        ...
        consignee_response = make_requests.make_request('POST', base_url=self.base_url,
    ...
    def add_third_party(self, draft_id, third_party):
        ...
        third_party_response = make_requests.make_request('POST', base_url=self.base_url,
