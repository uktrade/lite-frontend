...
class SeedData:
    ...
    def add_user(self, data, url, token_name):
        response = make_request('POST', url=url, body=data)
    ...
    def add_good(self):
        ...
        response = make_request('POST', url='/goods/', headers=self.export_headers, body=data)
    ...
    def add_clc_query(self):
        ...
        response = make_request("POST", url='/queries/control-list-classifications/', headers=self.export_headers, body=data)
    ...
    def add_clc_good(self):
        ...
        response = make_request('POST', url='/queries/control-list-classifications/', headers=self.export_headers,
                                     body=data)
    ...
    def add_eua_query(self):
        ...
        response = make_request("POST", url='/queries/end-user-advisories/', headers=self.export_headers, body=data)
        id = response.json()['end_user_advisory']['id']
        self.add_to_context('end_user_advisory_id', str(id))
        response = make_request("GET", url='/queries/end-user-advisories/' + str(id) + '/', headers=self.export_headers)
        self.add_to_context('end_user_advisory_case_id', response.json()['case_id'])

    def find_good_by_name(self, good_name):
        response = make_request('GET', url='/goods/', headers=self.export_headers)
    ...
    def add_org(self, key):
        ...
        response = make_request('POST', url='/organisations/', body=data)
    ...
    def add_case_note(self, context, case_id):
        ...
        make_request("POST", url='/cases/' + case_id + '/case-notes/', headers=self.gov_headers, body=data)  # noqa

    def add_ecju_query(self, case_id):
        self.log("Creating ecju query: ...")
        data = self.request_data['ecju_query']
        make_request("POST", url='/cases/' + case_id + '/ecju-queries/', headers=self.gov_headers, body=data)  # noqa

    def find_org_by_name(self, org_name):
        response = make_request('GET', url='/organisations/')
    ...
    def get_org_primary_site_id(self, org_id):
        response = make_request('GET', url='/organisations/' + org_id)
        organisation = response.json()['organisation']
        return organisation['primary_site']['id']

    def add_good_document(self, good_id):
        data = [self.request_data['document']]
        make_request("POST", url='/goods/' + good_id + '/documents/', headers=self.export_headers, body=data)

    def add_document(self, url):
        data = self.request_data['document']
        make_request("POST", url=url, headers=self.export_headers, body=data)
    ...
    def add_draft(self, draft=None, good=None, enduser=None, ultimate_end_user=None, consignee=None, third_party=None,
        ...
        response = make_request("POST", url='/drafts/', headers=self.export_headers, body=data)
        draft_id = response.json()['draft']['id']
        self.add_to_context('draft_id', draft_id)
        self.log("Adding site: ...")
        make_request("POST", url='/drafts/' + draft_id + '/sites/', headers=self.export_headers,
        ...
        end_user_post = make_request("POST", url='/drafts/' + draft_id + '/end-user/', headers=self.export_headers,
                          body=end_user_data)
        ...
        make_request("POST", url='/drafts/' + draft_id + '/goods/', headers=self.export_headers, body=data)
        ...
        ultimate_end_user_post = make_request('POST', url='/drafts/' + draft_id + '/ultimate-end-users/',
        ...
        consignee_response = make_request('POST', url='/drafts/' + draft_id + '/consignee/',
        ...
        third_party_response = make_request('POST', url='/drafts/' + draft_id + '/third-parties/',
        ...
        additional_documents_response = make_request('POST', url='/drafts/' + draft_id + '/documents/',
    ...
    def add_open_draft(self, draft=None):
        ...
        make_request("POST", url='/drafts/' + draft_id + '/countries/', headers=self.export_headers,
        ...
        make_request("POST", url='/goodstype/', headers=self.export_headers, body=data)
    ...
    def submit_application(self, draft_id=None):
        ...
        response = make_request('POST', url='/applications/', headers=self.export_headers, body=data)
    ...
    def add_queue(self, queue_name):
        ...
        response = make_request("POST", url='/queues/', headers=self.gov_headers, body=data)
        item = response.json()['queue']
        self.add_to_context('queue_id', item['id'])

    def get_queues(self):
        self.log("getting queues: ...")
        response = make_request("GET", url='/queues/', headers=self.gov_headers)
    ...
    def assign_case_to_queue(self, case_id=None, queue_id=None):
        ...
        make_request("PUT", url='/cases/' + case_id + '/', headers=self.gov_headers, body=data)

    def assign_test_cases_to_bin(self, bin_queue_id, new_cases_queue_id):
        self.log("assigning cases to bin: ...")
        response = make_request("GET", url='/queues/' + new_cases_queue_id + '/', headers=self.gov_headers)
        queue = response.json()['queue']
        cases = queue['cases']
        for case in cases:
            data = {'queues': [bin_queue_id]}
            make_request("PUT", url='/cases/' + case['id'] + '/', headers=self.gov_headers, body=data)

    def add_ecju_response(self, question, response):
        self.log("adding response to ecju: ...")
        case_id = self.context['case_id']
        ecju_queries = make_request("GET", url='/cases/' + case_id + '/ecju-queries/', headers=self.gov_headers)
        ecju_query_id = None
        ...
        make_request("PUT", url='/cases/' + case_id + '/ecju-queries/' + ecju_query_id + '/',
                          headers=self.export_headers, body=data)

    def check_document(self, url):
        response = make_request("GET", url=url, headers=self.export_headers)
    ...
    def add_ecju_query_picklist(self):
        ...
        response = make_request("POST", url='/picklist/', body=data)
