...
class DocumentTemplate(SeedClass):
    ...
    def get_paragraph(self, paragraph_id):
        return make_request(
            "GET", base_url=self.base_url, url="/picklist/" + str(paragraph_id) + "/", headers=self.gov_headers,
        ).json()["picklist_item"]

    def get_layouts(self):
        return make_request(
            "GET", base_url=self.base_url, url="/static/letter-layouts/", headers=self.gov_headers,
        ).json()["results"]
