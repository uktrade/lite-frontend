...
class Organisation(SeedClass):
    ...
    def find_org_by_name(self, org_name):
        organisations = make_request("GET", base_url=self.base_url, url='/organisations/?search_term=' + org_name,
    ...
    def add_org(self, key):
        ...
        return make_request('POST', base_url=self.base_url, url='/organisations/', body=data,
                            headers=self.gov_headers).json()['organisation']

    def get_org_primary_site_id(self, org_id):
        organisation = make_request('GET', base_url=self.base_url, url='/organisations/' + org_id,
