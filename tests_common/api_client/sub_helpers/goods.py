...
class Goods:
    ...
    def post_good(self, data):
        item = self.api_client.make_request(
            method="POST", url="/goods/", headers=ApiClient.exporter_headers, body=data,
        ).json()["good"]
    ...
    def find_good_by_name(self, good_name):
        goods = self.api_client.make_request(method="GET", url="/goods/", headers=ApiClient.exporter_headers).json()[
            "results"
        ]
