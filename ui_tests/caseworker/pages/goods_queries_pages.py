...
class GoodsQueriesPages(BasePage):
    ...
    def click_respond_to_clc_query(self):
        ...
        self.driver.find_element(by=By.ID, value=self.BUTTON_CLC_RESPOND_ID).click()

    def click_respond_to_grading_query(self):
        scroll_to_element_below_header_by_id(self.driver, self.BUTTON_GRADING_RESPOND_ID)
        self.driver.find_element(by=By.ID, value=self.BUTTON_GRADING_RESPOND_ID).click()
    ...
    def choose_report_summary(self):
        ...
        self.driver.find_element(by=By.ID, value=self.LINK_REPORT_SUMMARY_PICKLIST_PICKER_ID).click()
        self.driver.find_elements_by_class_name(self.LINK_PICKLIST_PICKER_ITEM_CLASS)[0].click()
        self.driver.find_element(by=By.ID, value=self.BUTTON_SUBMIT_REPORT_SUMMARY_ID).click()
    ...
    def select_a_grading(self, grading):
        Select(self.driver.find_element(by=By.ID, value=self.GRADING_ID)).select_by_visible_text(grading)
    ...
    def enter_a_comment(self, comment):
        comment_el = self.driver.find_element(by=By.NAME, value=f"{self.good_pk}{self.COMMENT}")
        comment_el.send_keys(comment)

    def enter_ars(self, ars_text):
        ars_el = self.driver.find_element(by=By.NAME, value=f"{self.good_pk}{self.REPORT_SUMMARY}")
    ...
    def good_pk(self):
        if not hasattr(self, "_good_pk"):
            step_input = self.driver.find_element(by=By.NAME, value=self.STEP_NAME)
