...
class F680AdditionalInformationPage(BasePage):
    ...
    def enter_date_of_issue(self, day, month, year):
        self.driver.find_element(by=By.ID, value=self.DATE_DAY_ID).send_keys(day)
        self.driver.find_element(by=By.ID, value=self.DATE_MONTH_ID).send_keys(month)
        self.driver.find_element(by=By.ID, value=self.DATE_YEAR_ID).send_keys(year)
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_no_date(self, value=False):
        self.driver.find_element(by=By.ID, value=self.EXPEDITED_SELECTOR_ID.format(value=value)).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_foreign_technology(self, value=False):
        self.driver.find_element(by=By.ID, value=self.FOREIGN_TECHNOLOGY_SELECTOR_ID.format(value=value)).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_locally_manufactured(self, value=False):
        self.driver.find_element(by=By.ID, value=self.LOCALLY_MANUFACTURED_SELECTOR_ID.format(value=value)).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_mtcr_type(self, value="mtcr_category_2"):
        self.driver.find_element(by=By.ID, value=self.MTCR_TYPE_SELECTOR_ID.format(value=value)).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_electronic_warfare_requirement(self, value=False):
        self.driver.find_element(
            by=By.ID, value=self.ELECTRONIC_WARFARE_REQUIREMENT_SELECTOR_ID.format(value=value)
        ).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_uk_service_equipment(self, value=False):
        self.driver.find_element(by=By.ID, value=self.UK_SERVICE_EQUIPMENT_SELECTOR_ID.format(value=value)).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_uk_service_equipment_type(self, value="mod_funded"):
        self.driver.find_element(by=By.ID, value=self.UK_SERVICE_EQUIPMENT_TYPE_SELECTOR_ID.format(value=value)).click()
        functions.click_submit(self.driver, self.submit_button_value)

    def enter_prospect_value(self, value="100.00"):
        self.driver.find_element(by=By.ID, value=self.PROSPECT_VALUE_SELECTOR_ID).send_keys(value)
        functions.click_submit(self.driver, self.submit_button_value)

    def click_expedited(self):
        self.driver.find_element(by=By.ID, value=self.EXPEDITED_ID).click()

    def click_foreign_technology(self):
        self.driver.find_element(by=By.ID, value=self.FOREIGN_TECHNOLOGY_ID).click()

    def click_locally_manufactured(self):
        self.driver.find_element(by=By.ID, value=self.LOCALLY_MANUFACTURED_ID).click()

    def click_mtcr_type(self):
        self.driver.find_element(by=By.ID, value=self.MTCR_TYPE_ID).click()

    def click_electronic_warfare_requirement(self):
        self.driver.find_element(by=By.ID, value=self.ELECTRONIC_WARFARE_REQUIREMENT_ID).click()

    def click_uk_service_equipment(self):
        self.driver.find_element(by=By.ID, value=self.UK_SERVICE_EQUIPMENT_ID).click()

    def click_uk_service_equipment_type(self):
        self.driver.find_element(by=By.ID, value=self.UK_SERVICE_EQUIPMENT_TYPE_ID).click()
