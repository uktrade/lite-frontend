...
class AddGood(LoginRequiredMixin, RegisteredFirearmDealersMixin, MultiFormView):
    ...
    def validate_step(self, request, nested_data):
        ...
        validation_response = validate_good(request, nested_data)

        logger.debug("AddGood.validate_step: validation_response=%s", validation_response)
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            validation_results, _ = validate_good(
                self.request,
                {"firearm_details": data_to_validate.copy()},
            )
