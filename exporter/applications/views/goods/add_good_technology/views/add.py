...
from exporter.goods.services import post_technology, post_good_documents
...
class AddGoodTechnology(
    ...
    def has_product_documentation(self):
        data = self.get_cleaned_data_for_step(AddGoodTechnologySteps.PRODUCT_DOCUMENT_UPLOAD)
    ...
    @expect_status(
        HTTPStatus.CREATED,
        "Error with product document when creating technology",
    ...
    @expect_status(
        HTTPStatus.CREATED,
        "Error creating technology product",
        "Unexpected error adding technology product",
    )
